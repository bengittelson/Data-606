...
**Insert your answer here**
The number of girls baptized generally increases from 1600 to 1640, followed by 
a decrease from 1640 to 1650. From 1650 to 1660 it increases and then decreases. 
From 1650 onwards, it generally increases, with periodic fluctuations. The rate 
of change of the number of baptisms appears to decrease starting a few years after 1680.
...
**Insert your answer here**
The proportion of boys born over time is always above 0.5, as indicated by the
plots below and the summary of the `more_boys` variable in the following 
section. Otherwise, there does not appear to be a clear trend in the proportion
of boys born over time in the line or scatter plots, and there is noticeable 
fluctuation from year to year. 

```{r plot-boy-ratio-vs-year-point}
ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + 
  geom_point()
```

```{r plot-boy-ratio-vs-year-line}
ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + 
  geom_line()
```
<div id="boxedtext">
...
**Insert your answer here**
The years included in this dataset range from 1940 through 2002. The dimensions 
of the data frame are (63, 3), i.e. 63 observations of 3 variables. The column 
names are "year," "boys," and "girls."
```{r summarize min and max for present}
present %>%
  summarize(min = min(year), max = max(year))
```

```{r get column names and dimensions for present by printing dataframe}
present
```

```{r get column names and dimensions for present by printing colnames and dim}
colnames(present)
dim(present)
```
...
```{r compare-summary-statistics-boys}
summary(arbuthnot$boys)
summary(present$boys)
```

```{r compare-summary-statistics-girls}
summary(arbuthnot$girls)
summary(present$girls)
```

```{r compare-summary-statistics-totals}
present <- present %>%
  mutate(total = boys + girls)
...
```{r compare-histograms-totals}
p <- ggplot() + geom_histogram(aes(x = arbuthnot$total, fill='Arbuthnot')) +
  geom_histogram(aes(x = present$total, fill='Present')) + labs(title="Total births per year for Arbuthnot and Present",
        x ="Total yearly births", y = "Count", fill='Dataset')
...
**Insert your answer here**
The counts are not of a similar order of magnitude, as evidenced by the 
summaries of the counts of boy, girl, and total births and the histogram of 
total yearly births above. For example, the median and mean total births for the 
Arbuthnot dataset are on the order of `10^4` (11813 and 11442, respectively) and
on the order of `10^6` (3756547 and 3679515, respectively) for the Present 
dataset. 
...
**Insert your answer here**
* `boy_ratio` is also above 0.5 every year in the Present dataset, which 
indicates ethat Arbuthnot's observation about boys being born in greater 
proportions than girls does hold up in the U.S.

```{r calc-total-birth-vars-save-present}
# add columns needed to answer this question and print head of dataframe
...
```{r plot-boy-ratio-vs-year-point-present}
ggplot(data = present, aes(x = year, y = boy_ratio)) + 
  geom_point()
```

```{r plot-boy-ratio-vs-year-line-present}
ggplot(data = present, aes(x = year, y = boy_ratio)) + 
  geom_line()
```
...
**Insert your answer here**
I added the totals column in the exercise above. We saw the greatest number of 
births in 1961. 
